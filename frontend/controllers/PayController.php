...
class PayController extends Controller
{
        ...
        return [
            'access' => [
                ...
                'rules' => [
                    ...
                    [
                        'actions' => ['caller-book-success', 'update-status'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                ],
    ...
    public function actionCallerBookSuccess($id) {
        $id = (int)$id;
        $pay = Pay::findAcModel($id);
        $user = Yii::$app->user->identity;
        /* @var $user \common\models\User */
        if(!$pay || $pay->user_id!=$user->id) {
            throw new \Exception('支付参数无效');
        }

        if($pay->status==Pay::STATUS_PAY_SUCCESS) {
            return $this->redirect(['/call/order/user']);
        }

        if($pay->expire_time<=time()) {
            $pay->updateAttributes([
                'status'=>Pay::STATUS_EXPIRE
            ]);
        }

        $orderIds = $dates = [];
        $orderPays = OrderPay::find()->where('pay_id='.(int)$pay->id)->all();
        foreach($orderPays as $orderPay) {
            $orderIds[] = (int)$orderPay->order_id;
        }
        $orders = Order::findAllAcModels($orderIds);
        foreach($orders as $order) {
             $dates[] = date('m-d', $order->booking_date);
        }
        $payTitle = implode(',', $dates).' 起床服务预订';
        $payJsParams = $this->getJsApiParams($pay, $payTitle);
        if($payJsParams['code']!==0) {
            return Response::renderWebErrorPage('支付失败', '获取参数失败', [
                ['url'=> Url::current(),'text'=>'刷新']
            ]);
        }
        return $this->render('caller-book-success', [
            'pay'=>$pay,
            'orders'=>$orders,
            'dates'=>$dates,
            'jsApiParams'=> $payJsParams['data'],
        ]);
    }
...
//        $account = Account::getUserAccount($user->id, Account::TYPE_WECHAT);
        $account = TMember::find()->where(['id'=>$user])->asArray()->one();
        if(!$account) {
            throw new \Exception('找不到和你关联的微信号信息');
        }

        $openIdRecord = UnionId::getAccount($account->union_id, $mp->id);
        if(!$openIdRecord) {
...
PAYERROR--支付失败(其他原因，如银行返回失败)
        ...
        return $this->redirect(['caller-book-success','id'=>$id]);
    }
