...
$this->title = '设置微信菜单';
$this->params['breadcrumbs'][] = ['label' => '设置菜单', 'url' => ['index']];
$this->params['breadcrumbs'][] = $this->title;

$this->registerCss('.wechat-dialog {
    background: #EBEBEB;
    width:380px;
    height: 400px;
    position: relative;
...
.wechat-menu ul,.wechat-menu li {
    list-style: none;
    padding:0;
}');
?>
<div class="wechat-setmenu">
    <h1><?= Html::encode($this->title) ?></h1>

    <div class="wechat-dialog" id="wechat_dialog">
        <div class="wechat-dialog-header">
            微信对话窗口
        </div>
        <div id="wechat_menus" class="clearfix wechat-menus"></div>
    </div>
    <div class="actions">
        <a href="#" class="btn btn-primary" id="btn_add_wm">增加一级菜单</a>
        <a href="#" class="btn btn-primary" id="btn_submit_wm">保存</a>
    </div>

</div>
<script type="text/javascript">
    <?php
    $this->beginBlock('pageScripts');
    ?>
var WechatMpMenu = {
    i:1,
    menus: {},
...
var menus = $.parseJSON('<?=json_encode($menus,JSON_UNESCAPED_UNICODE)?>');
    var allowEventKeys = {//允许的事件key
        <?php
        $allowEventKeys = [];
        $i = 0;
        $count = count($allowEventKeys);
        foreach($allowEventKeys as $key=>$allowEventKey) {
            $i++;
            echo "$key:'$allowEventKey'".($i==$count ? '' : ',');
        }
        ?>
    };
    
    WechatMpMenu.init(menus, allowEventKeys, function(menus){
            if(WechatMpMenu.submitting) {
                alert('正在提交数据...请稍后再试');
                return false;
            }
            WechatMpMenu.submitting = true;
            $.ajax({
                url:'/wechat/mp/save-menu?id=<?=$id?>',
                dataType: 'json',
            type: 'POST',
            data: {
                menus: menus
