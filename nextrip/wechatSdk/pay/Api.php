...
class Api {
    ...
    public static function unifiedOrder($inputObj, $timeOut = 6) {
        ...
        if (!$inputObj->IsNotify_urlSet()) {
            $inputObj->SetNotify_url($inputObj->getConfig()->getNotifyUrl()); //异步通知url
        }
        ...
        $response = self::postXmlCurl($xml, $url, $inputObj->getConfig(), false, $timeOut);
        $result = Results::Init($response, $inputObj->getConfig());
        self::reportCostTime($url, $startTimeStamp, $result, $inputObj->getConfig()); //上报请求花费时间
    ...
    public static function refund($inputObj, $timeOut = 6) {
        ...
        $xml = $inputObj->ToXml();
        $startTimeStamp = self::getMillisecond(); //请求开始时间
        $response = self::postXmlCurl($xml, $url, $inputObj->getConfig(), true, $timeOut);
        $result = Results::Init($response, $inputObj->getConfig());
        self::reportCostTime($url, $startTimeStamp, $result, $inputObj->getConfig()); //上报请求花费时间
    ...
    public static function notify($callback, &$msg, $config) {
        ...
        //如果返回成功则验证签名
        try {
            $result = Results::Init($xml, $config);
        } catch (Exception $e) {
            $msg = $e->errorMessage();
            return false;
        }
    ...
    private static function reportCostTime($url, $startTimeStamp, $data, $config) {
        //如果不需要上报数据
        if ($config->getReportLevel() == 0) {
            return;
        }
        //如果仅失败上报
        if ($config->getReportLevel() == 1 &&
     ...
     * @throws Exception
     */
    private static function postXmlCurl($xml, $url, $config, $useCert = false, $second = 30) {
        $ch = curl_init();
        //设置超时
        ...
        if ($config->getCurlProxyHost() != "0.0.0.0" && $config->getCurlProxyPort() != 0) {
            curl_setopt($ch, CURLOPT_PROXY, $config->getCurlProxyHost());
            curl_setopt($ch, CURLOPT_PROXYPORT, $config->getCurlProxyPort());
        }
        ...
        if ($useCert == true) {
            ...
            curl_setopt($ch, CURLOPT_SSLCERT, $config->getSslCertPath());
            curl_setopt($ch, CURLOPT_SSLKEYTYPE, 'PEM');
            curl_setopt($ch, CURLOPT_SSLKEY, $config->getSslKeyPath());
        }
        //post提交方式
